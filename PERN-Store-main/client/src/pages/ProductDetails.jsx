...
const ProductDetails = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      setIsLoading(true);
      const { data: reviewsData } = await reviewService.getReviews(id);
      console.log(reviewsData);
      setReviews(reviewsData.reviews);
      setReviewExist(reviewsData.reviewExist);
      setIsLoading(false);
    }
    fetchData();
  }, [id]);
  ...
  return (
    <Layout loading={isLoading}>
      <section className="body-font overflow-hidden">
        ...
        <div className="container w-full my-15">
          <ReviewModal product_id={id} reviews={{ reviews, reviewExist }} />
        </div>
